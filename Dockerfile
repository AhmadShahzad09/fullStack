...
#### Install Maven
RUN export JAVA_HOME && wget https://dlcdn.apache.org/maven/maven-3/3.8.4/binaries/apache-maven-3.8.4-bin.tar.gz && mkdir -p /usr/local/apache-maven && mv apache-maven-3.8.4-bin.tar.gz /usr/local/apache-maven && cd /usr/local/apache-maven && tar -xzvf apache-maven-3.8.4-bin.tar.gz
...
RUN export M2_HOME && export M2 && export MAVEN_OPTS && export PATH  && wget https://repo.spring.io/release/org/springframework/boot/spring-boot-cli/2.3.0.RELEASE/spring-boot-cli-2.3.0.RELEASE-bin.tar.gz && tar -xzf spring-boot-cli-2.3.0.RELEASE-bin.tar.gz && mv spring-2.3.0.RELEASE /opt/ && ln -s /opt/spring-2.3.0.RELEASE/bin/spring /usr/bin/spring && ln -s /opt/spring-boot-cli-2.3.0.RELEASE/shell-completion/bash/spring /etc/bash_completion.d/spring | sh
...
RUN npm install -y @angular/cli


EXPOSE 4200

CMD ["cd", "fx-deals/src/fx-ui", "&&", "npm", "start"]
